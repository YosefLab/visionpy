...
function set_global_status(update) {
    ...
    if ("plotted_item" in update || "plotted_item_type" in update) {
        ...
        if (
            ...
            var sig_info_promise = api.signature
                .info(new_sig)
                .then((new_info) => {
    ...
    if ("enrichment" in update || "enrichment_module" in update) {
        ...
        if (enriched) {
            var mod_sig_promise = api.modules
                .hotspot_score(get_global_status("enrichment_module"))
                .then((new_info) => {
            ...
            var mod_gene_list_promise = api.modules
                .mod_gene_list(get_global_status("enrichment_module"))
                .then((new_info) => {
...
window.onload = function () {
    ...
    var allCellClustersPromise = api.clusters
        .meta_levels()
        .then(function (data) {
    ...
    var combinedPromise = $.when(
        sessionInfoPromise,
        allCellClustersPromise,
    ).then(function () {
        return upper_left_content.init();
    });

    var combinedPromise2 = $.when(
        sessionInfoPromise,
        allCellClustersPromise,
    ).then(function () {
    ...
    $.when(
        ...
        if (has_tree) {
            // need to change styling for navbar button to select Traj
