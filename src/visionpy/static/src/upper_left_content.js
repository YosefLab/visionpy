...
Upper_Left_Content.prototype.init = function () {
    ...
    for (var i = 0; i < cluster_variables.length; i++) {
        clust_dropdown.append(
            $("<option />")
                .val(cluster_variables[i])
                .text(cluster_variables[i]),
        );
    }
    clust_dropdown.on("change", function () {
        self.update({
    ...
    $(this.dom_node)
        ...
        .on("click", function (e) {
            if (
                e.target.id === "genes-table-tab" ||
                e.target.id === "de-table-tab"
            ) {
...
Signature_Table.prototype.init = function () {
    ...
    var outPopper = new Popper(initialNode, popper, {
        placement: "top",
        modifiers: {
            arrow: {
                enabled: true,
...
Signature_Table.prototype.render = function () {
    ...
    var cluster_ids = _(self.clusters)
        .values()
        .uniq()
        .sort((x) => x)
        .value();
    ...
    var header_row = $(self.dom_node)
        .children(".sig-tables-header")
        .find(".proj-row");
    ...
    var clusterTableDiv = $(self.dom_node)
        .children(".sig-tables-wrapper")
        .first();
    var new_table_divs = [];
    ...
    for (var i = 0; i < cluster_ids.length; i++) {
        ...
        var sig_labels = matrix.sig_labels.filter(function (x) {
            return self.is_filtering || self.clusters[x] == curr_cl;
        });
        ...
        var formatted_data_matrix = _.zip(sig_indices, zscores, pvals).map(
            (x) => {
                var sig_index = x[0];
                var zscores_row = x[1];
                var pvals_row = x[2];
        ...
        var formatted_data_w_row_labels = d3.zip(
            sig_labels,
            formatted_data_matrix,
        );
        ...
        if (cluster_ids.length > 1) {
            ...
            if (sort_col > -1) {
                $(new_table_div).data(
                    "table-sort-val",
                    leader_row[1][sort_col].zscore * -1,
                );
            }
        }
        ...
        var colorScaleCluster = d3.scale
            .linear()
            .domain([0, 0.5, 1])
            .range(["steelblue", "white", "lightcoral"])
            .clamp(true);
        ...
        var content_row = content_rows.selectAll("td").data(function (d) {
        ...
        if (main_vis === "pcannotator") {
            content_row
                ...
                .on("click", function (d) {
                    tableClickFunction_PC(
                        matrix.sig_labels[d.row],
                        "signature",
                    );
                })
                .append("div");
            ...
            content_row
                ...
                .append("div")
                .text(function (d) {
                    return d.zscore.toPrecision
                        ? d.zscore.toPrecision(2)
                        : d.zscore;
                });
            ...
            content_row
                ...
                .style("background-color", function (d) {
                    return colorScaleCluster(d.zscore);
                })
                .append("div")
                .text(function (d) {
                    return d.pval < 0.05 ? "*" : "";
        ...
        content_row
            ...
            .html(function (d) {
                return '<div class="sig-row-label"><div>' + d + "</div></div>";
            });
...
Signature_Table.prototype.update = function (updates) {
    ...
    if (_.isEmpty(self.clusters)) {
        clusters_promise = api.signature.clusters(false).then(function (cls) {
            self.clusters = cls;
            return true;
        });
    } else {
    ...
    if (
        "main_vis" in updates ||
        _.isEmpty(self.matrix) ||
        "cluster_var" in updates
    ) {
        var main_vis = get_global_status("main_vis");
        ...
        matrix_promise = matrix_promise.then(function (matrix) {
            if (fix_col_label) {
    ...
    return $.when(matrix_promise, clusters_promise).then(
        function (matrix_update, clusters_update) {
    ...
    for (var i = 0; i < cluster_ids.length; i++) {
        ...
        if (main_vis === "pcannotator") {
            content_row
                ...
                .on("click", function (d) {
                    tableClickFunction_PC(matrix.sig_labels[d.row], "protein");
                })
                .append("div");
        } else {
            content_row
                ...
                if (main_vis === "pcannotator") {
                    ...
                    if (main_vis === "clusters" && i > 0)
                        tooltip_str =
                            ...
                            " p<" +
                            _pval_format(d.pval);
                    else tooltip_str = "C'=" + d.zscore.toFixed(2);
                }
                createTooltip(self.tooltip, this, tooltip_str);
                hoverRowCol(header_row, this, matrix.proj_labels[d.col]);
            };
...
Protein_Table.prototype.update = function (updates) {
    ...
    var clusters_promise;
    if (_.isEmpty(self.clusters)) {
        clusters_promise = api.protein.clusters().then(function (cls) {
            self.clusters = cls;
            return true;
        });
    } else {
...
Meta_Table.prototype.update = function (updates) {
    ...
    var clusters_promise;
    if (_.isEmpty(self.clusters)) {
        clusters_promise = api.signature.clusters(true).then(function (cls) {
            self.clusters = cls;
            return true;
        });
    } else {
...
Meta_Table.prototype.render = function () {
    ...
    header_row.find("th:first-child").on("click", function () {
    ...
    for (var i = 0; i < cluster_ids.length; i++) {
        ...
        if (main_vis === "pcannotator") {
            content_row
                ...
                .on("click", function (d) {
                    tableClickFunction_PC(matrix.sig_labels[d.row], "meta");
                })
                .append("div");
        } else {
            content_row
...
Gene_Select.prototype.init = function () {
    var gene_promise = api.expression.genes.list().then(function (genes) {
...
DE_Select.prototype.init = function () {
    ...
    table
        .buttons()
        .container()
        .prependTo($(this.dom_node).find(".button-container"));

    $(this.dom_node)
        .find(".fdr-holder")
        .remove()
        .appendTo($(this.dom_node).find(".fdr-filter"));
        ...
        if (numControl.val() === "selections") {
            if (numSelect.val() === "current") {
                ...
                if (group_num.length === 0) {
                    de_error.html(
                        "No cells currently selected<br>Make a selection using the lasso tool on the scatter plot",
                    );
                    de_error.show();
                    return;
                }
                ...
                if (group_num === null) {
                    de_error.html(
                        "No saved selections<br>To save a selection, first select cells on the scatter plot using the lasso tool.  Then click 'Save' above the plot and give the selection a name.",
                    );
                    de_error.show();
                    return;
                }
...
DE_Select.prototype.render_de = function () {
    ...
    var dataSet = _.map(features, (f, i) => [
        features[i],
        types[i],
        logFCs[i],
        stats[i],
...
Signature_Table.prototype.doneTyping = function () {
    ...
    var posvals = vals.filter(function (str) {
        return str[0] != "!";
    });
    var negvals = vals
        .filter(function (str) {
