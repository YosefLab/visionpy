...
Lower_Left_Content.prototype.update = function (updates) {
    ...
    if ("plotted_item_type" in updates) {
        if (
            updates["plotted_item_type"] !== "signature-gene" &&
    ...
    if ("selection_type" in updates) {
        ...
        if (SELECTION_IS_CELL) {
            ...
            !SELECTION_IS_CELL &&
            $(this.nav["cell_info"]).hasClass("active")
        ) {
...
Lower_Left_Content.prototype.resize = _.debounce(
    Lower_Left_Content.prototype._resize,
    300,
);
...
Values_Plot.prototype.update = function (updates) {
    ...
    if (item_type === "gene" || item_type === "signature-gene") {
        var urllink =
            "http://www.genecards.org/cgi-bin/carddisp.pl?gene=" + item;
        $(this.title).html(
...
Values_Plot.prototype.plot = function () {
    ...
    if (
        ...
        var selected_values = _.values(
            _.pick(plotted_values_object, selected_cells),
        );
        var remainder_values = _.values(
            _.omit(plotted_values_object, selected_cells),
        );
        drawDistChartSelection(
            this.chart,
            selected_values,
            remainder_values,
            selection_name,
        ...
        columns: [
            {
                ...
                render: function (data) {
                    return (
                        '<a href="javascript:void(0);" onclick="_setSignatureGene(\'' +
                        data +
                        "')\">" +
                        data +
                        "</a>"
...
Sig_Info.prototype.update = function (updates) {
    ...
    dt.DataTable().clear().rows.add(dataSet).draw();
};
...
Sig_Heatmap.prototype.drawHeat = function () {
    ...
    return $.when(api.signature.expression(sig_key, cluster_var))
        .then(function (sig_expression) {
            ...
            var options = {
                ...
                modeBarButtons: [
                    [],
                    ["resetScale2d", "zoom2d", "pan2d", "toImage"],
                ],
            };
...
Cell_Info.prototype.update = function (updates) {
    ...
    return api.cell.meta(selected_cell).then((result) => {
        ...
        var dataSet = _.map(result, function (v, k) {
            return [k, v];
        });

        self.cell_info_table.DataTable().clear().rows.add(dataSet).draw();
    });
};
...
Selection_Info.prototype.init = function () {
    ...
    this.selection_info_table_meta = $(this.dom_node).find(
        "#selection-info-table-meta",
    );
    ...
    var _metaDetailsFormat = function (data) {
        var result =
            '<table style="margin-right: auto; margin-left: auto;" class="meta-expand-table">';
        result =
            result +
    ...
    this.selection_info_table_meta.on(
        "click",
        "td.details-control",
        function () {
...
Selection_Info.prototype.update = function (updates) {
    ...
    return api.cells.meta(selected_cells).then((result) => {
        ...
        self.selection_info_table.DataTable().clear().rows.add(dataSet).draw();

        var dataSetMeta = _.map(result.factor, function (value, property) {
            var levels = _(value)
                .map(function (percent, level) {
        ...
        self.selection_info_table_meta
            .DataTable()
            .clear()
...
function drawDistChart(node, values, logScale) {
    var isFactor = typeof values[0] === "string" && values[0] !== "NA";
...
function drawDistChartSelection(
    node,
    selected_values,
    remainder_values,
    selection_name,
    ...
    var isFactor =
        typeof selected_values[0] === "string" && selected_values[0] !== "NA";
